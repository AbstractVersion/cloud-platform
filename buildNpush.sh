...
read -p "Build & Push FILESYSTEM_SERVICE ? (y/n) " RESP
if [ "$RESP" = "y" ]; then
    # zuul-gateway 
    echo -------------------------zuul-gateway Build--------------------------
    cd filesystem-service && mvn clean install -DskipTests
    # push the image to a local repo
    docker tag $registry_ip/filesystem-service:latest $registry_ip/filesystem-service:production
    docker push $registry_ip/filesystem-service:production
    cd ..
else
    echo "Ok then proceeding with the initialization..."
fi

read -p "Build & Push CLONER-SERVICE ? (y/n) " RESP
if [ "$RESP" = "y" ]; then
    # zuul-gateway 
    echo -------------------------clone-service Build--------------------------
    cd clone-service && mvn clean install -DskipTests
    # push the image to a local repo
    docker tag $registry_ip/clone-service:latest $registry_ip/clone-service:production
    docker push $registry_ip/clone-service:production
    cd ..
...
read -p "Build & Push DIRECTORY-SERVICE ? (y/n) " RESP
if [ "$RESP" = "y" ]; then
    # zuul-gateway 
    echo -------------------------zuul-gateway Build--------------------------
    cd directory-service && mvn clean install -DskipTests
    # push the image to a local repo
    docker tag $registry_ip/directory-service:latest $registry_ip/directory-service:production
    docker push $registry_ip/directory-service:production
    cd ..
else
    echo "Ok then proceeding with the initialization..."
fi

read -p "Build & Push CONNECTOR-SERVICE ? (y/n) " RESP
if [ "$RESP" = "y" ]; then
    # zuul-gateway 
    echo -------------------------connector-service Build--------------------------
    cd infinite-connector/connector-service/ && docker image build -t $registry_ip/connector-api:latest .
    # push the image to a local repo
    docker tag $registry_ip/connector-api:latest $registry_ip/connector-api:production
    docker push $registry_ip/connector-api:production
    cd ../..
else
    echo "Ok then proceeding with the initialization..."
fi

read -p "Build & Push CONNECTOR-WORKER ? (y/n) " RESP
if [ "$RESP" = "y" ]; then
    # zuul-gateway 
    echo -------------------------connector-worker Build--------------------------
    cd infinite-connector/connector-worker/ && docker image build -t $registry_ip/connector-worker:latest .
    # push the image to a local repo
    docker tag $registry_ip/connector-worker:latest $registry_ip/connector-worker:production
    docker push $registry_ip/connector-worker:production
    cd ../..
else
    echo "Ok then proceeding with the initialization..."
fi
